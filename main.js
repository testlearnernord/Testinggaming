...
const RESERVED_AREA_KEYS = ["fieldArea", "yardArea", "pondArea"];
...
function isReservedTile(tx, ty) {
  if (!state.map) return false;
  for (const key of RESERVED_AREA_KEYS) {
    const area = state.map[key];
...
function resizeCanvas() {
  ...
  const cssWidth = parent?.clientWidth ?? window.innerWidth ?? canvas.width / dpr ?? 960;
  const cssHeight = parent?.clientHeight ?? window.innerHeight ?? canvas.height / dpr ?? 540;
  const width = Math.max(640, cssWidth);
  const height = Math.max(360, cssHeight);
...
function maybeEnterFullscreen(key) {
  if (state.fullscreenAttempted) return;
...
function updateMouseFromEvent(ev) {
  ...
  const width = rect.width || canvas.width / state.dpr;
  const height = rect.height || canvas.height / state.dpr;
  const x = ev.clientX - rect.left;
  const y = ev.clientY - rect.top;
  state.mouse.x = x;
  state.mouse.y = y;
  state.mouse.worldX = state.camera.x - width / 2 + x;
  state.mouse.worldY = state.camera.y - height / 2 + y;
}
...
function updatePlayer(dt) {
  ...
  if (moving) {
    ...
    const aimX = state.mouse.worldX - player.x;
    const aimY = state.mouse.worldY - player.y;
    ...
    if (aimLen > 2) {
      ...
      player.dir.x = nx;
      player.dir.y = ny;
      player.anim.facing = vectorToFacing(nx, ny, player.anim.facing);
    }
  }
...
function readMovementInput() {
  ...
  const sprintKey = isAnyPressed(CONTROLS.keyboard.sprint);
  return { x, y, magnitude: Math.min(magnitude, 1), sprint: sprintKey };
}
...
function computeView(width, height) {
  ...
  const left = state.camera.x - halfW - margin;
  const right = state.camera.x + halfW + margin;
  const top = state.camera.y - halfH - margin;
  const bottom = state.camera.y + halfH + margin;
  ...
  return {
    ...
    top,
    bottom,
    tileMinX: Math.max(0, Math.floor(left / TILE)),
    tileMaxX: Math.min(cols, Math.ceil(right / TILE)),
    tileMinY: Math.max(0, Math.floor(top / TILE)),
    tileMaxY: Math.min(rows, Math.ceil(bottom / TILE)),
  };
}
...
function drawTiles(view) {
  const map = state.map;
  if (!map) return;
  const startY = Math.max(0, view?.tileMinY ?? 0);
  const endY = Math.min(map.rows - 1, view?.tileMaxY ?? map.rows - 1);
  const startX = Math.max(0, view?.tileMinX ?? 0);
  const endX = Math.min(map.cols - 1, view?.tileMaxX ?? map.cols - 1);
...
function drawHouses(view) {
  for (const house of HOUSES) {
    if (view && !houseInView(house, view)) continue;
    drawHouse(house);
  }
}

function houseInView(house, view) {
  const { area } = house;
  const left = area.x * TILE;
  const top = area.y * TILE;
  const right = left + area.w * TILE;
  const bottom = top + area.h * TILE;
...
function drawPlants(view) {
  for (const plant of state.plants.values()) {
    if (view) {
      if (plant.tx < view.tileMinX - 1 || plant.tx > view.tileMaxX + 1) continue;
      if (plant.ty < view.tileMinY - 1 || plant.ty > view.tileMaxY + 1) continue;
    }
...
function drawDirt(view) {
  ...
  const bounds = view || null;
  for (const clod of state.dirt) {
    if (bounds) {
      const radius = clod.radius + TILE * 0.5;
      if (clod.x + radius < bounds.left || clod.x - radius > bounds.right) continue;
      if (clod.y + radius < bounds.top || clod.y - radius > bounds.bottom) continue;
    }
    ctx.beginPath();
...
function drawStones(view) {
  ...
  for (const stone of state.stones) {
    if (bounds) {
      const radius = stone.radius + TILE * 0.5;
      if (stone.x + radius < bounds.left || stone.x - radius > bounds.right) continue;
      if (stone.y + radius < bounds.top || stone.y - radius > bounds.bottom) continue;
    }
    ctx.beginPath();
...
function drawNPCs(view) {
  ...
  for (const npc of state.npcs) {
    if (view) {
      const margin = TILE * 1.5;
      if (npc.x + margin < view.left || npc.x - margin > view.right) {
        index += 1;
        continue;
      }
      if (npc.y + margin < view.top || npc.y - margin > view.bottom) {
        index += 1;
        continue;
      }
    }
...
function drawPlayer(view) {
  if (view) {
    const margin = TILE * 1.5;
    if (state.player.x + margin < view.left || state.player.x - margin > view.right) return;
    if (state.player.y + margin < view.top || state.player.y - margin > view.bottom) return;
  }
  const player = state.player;
...
function drawPreview(view) {
  ...
  if (!preview) return;
  if (view) {
    if (preview.tx < view.tileMinX - 1 || preview.tx > view.tileMaxX + 1) return;
    if (preview.ty < view.tileMinY - 1 || preview.ty > view.tileMaxY + 1) return;
  }
  ctx.save();
