...
Top-down Farm-Arcade für GitHub Pages. Sammle Felsen, wandle sie in 💩 um und baue Mais oder Kohl an. Die Build richtet sich jetzt an Desktop-Browser, nutzt Canvas 2D und speichert automatisch unter `pb_save_v7` im `localStorage`.
...
4. App im Browser öffnen. Beim ersten Klick wird Audio freigeschaltet.

## Neu in v1.3.1

- Desktop-Steuerung ohne Mobile-Overlays: Touch-Joystick, Sprint-Button und andere Smartphone-Elemente wurden entfernt. Bewegung läuft über WASD/Pfeiltasten, Aktionen über Leertaste oder Linksklick.
- Mausausrichtung: Der Spieler schaut zur Maus, auch im Stand. Links-Klick löst kontextabhängige Aktionen aus.
- Stabileres Canvas-Boot: Die Canvas nutzt jetzt eine einfache Desktop-Layout-Logik und vermeidet so schwarze Bildschirme durch Touch-spezifische Initialisierung.
...
- **Leertaste / Linksklick** – Kontextaktion (Shop, Pflanzen, Abliefern, Editor)
...
- Optional Tag setzen (`git tag v1.3.1`).
...
**Kein Sound:**
- Der erste User-Input schaltet Audio via `primeAudioUnlock()` frei. Bei blockierenden Browsern einmal klicken.
...
- Start ohne Konsolenfehler im Desktop-Browser
- HUD zeigt Version `v1.3.1` im Overlay an
